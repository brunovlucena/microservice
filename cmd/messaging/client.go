...
type IMessagingClient interface {
	...
	Subscribe(exchangeName string, exchangeType string, consumerName string) (chan *Config, error)
	SubscribeCR(exchangeName string, exchangeType string, consumerName string) (chan *ConfigRequest, error)
	Close()
}
...
func (m *MessagingClient) Subscribe(exchangeName string, exchangeType string, consumerName string) (chan *Config, error) {
	if m.conn == nil {
		panic("Tried to send message before connection was initialized. Don't do that.")
	}
...
func consumeLoop(deliveries <-chan amqp.Delivery) chan *Config {
...
func (m *MessagingClient) SubscribeCR(exchangeName string, exchangeType string, consumerName string) (chan *ConfigRequest, error) {
	if m.conn == nil {
		panic("Tried to send message before connection was initialized. Don't do that.")
	}
	...
	c := consumeLoopCR(msgs)

	return c, nil

}

func consumeLoopCR(deliveries <-chan amqp.Delivery) chan *ConfigRequest {
	configRequest := make(chan *ConfigRequest)
	go func() {
		for d := range deliveries {
